...
install_openvz() {
	...
	yum install vzctl vzquota vzkernel ploop -y
	sed -i 's/#net.ipv4.ip_forward = 1/net.ipv4.ip_forward = 1/g' /etc/sysctl.conf
	sed -i 's/#kernel.sysrq = 1/kernel.sysrq = 1/g' /etc/sysctl.conf
	sysctl -p
...
resolve_deps() {
  ...
  if [ "$DISTRIB_ID" = "RedHat" -o "$DISTRIB_ID" = "CentOS" ]; then
    yum -y install ruby-devel ruby-docs ruby-ri ruby-irb ruby-rdoc ruby-devel
...
install_product() {
  puts "Installation..."
  yorn="yes"
  echo "Do you want to install OpenVZ? Please insert yes or no. [Default: yes]"
  read yorn
   if [ $yorn = "yes" ]; then
      install_openvz
   else
...
print_access_info() {
  if [ $yorn = "yes" ]; then
	puts "Panel should be available at:"
 	puts "http://`curl http://ifconfig.me`:3000"
	puts "Default credentials: admin/admin"
	puts "Because you choose to install OpenVZ, reboot is required."
	puts "Restart will begin within 3 seconds."
	sleep 3
	reboot
  else
	puts "Panel should be available at:"
